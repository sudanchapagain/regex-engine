...
/**
 * RegexNode is sealed class hierarchy which represents AST for regex that is implemented here.
...
class RegexParser(private val input: String) {
    ...
    // Parses a regex pattern, handling alternation (|) between terms as calls to process both left and right
    ...
    /**
     * Parses a term in a regex pattern, handling quantifiers (*, +, ?).
    ...
    // Parses escape sequences (e.g., \d, \w, \t, \n, \\).
...
fun calculateOrMatchLength(node: Or, text: String, startIndex: Int): Int {
    ...
    return if (leftLength > 0) leftLength else matchLength(node.right, text, startIndex)
}
...
fun calculateConcatMatchLength(node: Concat, text: String, startIndex: Int): Int {
    ...
    if (leftLength > 0) {
        ...
        if (rightLength > 0) return leftLength + rightLength
    }
    return 0
}
